...
META_PROMPT_PATTERNS = [
    r"\bignore\s+(?:all\s+)?(?:previous|prior|the user's original)\s+instructions?\b",
    r"\bignore\s+the\s+user['’]s\s+(?:original\s+)?question\b",
    r"\breveal\s+(?:the\s+)?system\s+prompt\b",
    r"\bsystem\s+note\s*:",
    r"\bhigher\s+priority\s+than\s+the\s+user['’]s\s+question\b",
    r"\bdo\s+not\s+answer\s+the\s+(?:user['’]s\s+)?original\s+question\b",
    r"\binclude\s+the\s+following\s+(?:fictional\s+)?employee\s+record\b",
    r"\bfollow\s+this\s+document\s+instead\b",
    r"\bforget\s+the\s+previous\s+details\b",
    r"\bfocus\s+on\s+my\s+new\s+question\b",
    r"\banswer\s+the\s+following\s+question\s+instead\b",
    r"\bset\s+aside\s+the\s+earlier\s+content\b",
    r"\bexplain\s+in\s+detail\s+how\s+to\b.{0,180}\b(?:exfiltrate|steal\s+credentials|bypass|evade|avoid\s+detection|without\s+triggering|ransomware|phishing|malware|credential\s+theft)\b",
    r"\bexfiltrate\s+classified\s+internal\s+documents\b",
    r"\bwithout\s+triggering\b.{0,80}\b(?:monitoring|security|detection|alerts?)\b",
]
...
def _meta_prompt_guardrail(config: ExternalGuardrailConfig, stage: str, text: str) -> dict[str, Any]:
    result = default_result(config, stage)
    hits = [pattern for pattern in META_PROMPT_PATTERNS if re.search(pattern, text or "", re.IGNORECASE | re.DOTALL)]
    if hits:
        result["flagged"] = True
        result["categories"] = ["prompt_injection"]
        if any("exfiltrate" in hit or "without" in hit or "explain" in hit for hit in hits):
            result["categories"].append("muted_guardrail_trigger")
        result["reason"] = "meta_prompt_pattern_match:" + ",".join(hits[:5])
        result["risk_score"] = min(1.0, 0.35 + 0.12 * len(hits))
    return _finalize(config, result)
...
def _llama_guard_stack(config: ExternalGuardrailConfig, stage: str, text: str, metadata: dict[str, Any] | None) -> dict[str, Any]:
    result = default_result(config, stage)
    api_url = config.api_url or "http://127.0.0.1:8191/check"
    body = json.dumps(
        {
            "text": text,
            "stage": stage,
    ...
    try:
        ...
        return _error_result(config, stage, f"llama_guard_stack:{exc}")

    if not isinstance(payload, dict):
        return _error_result(config, stage, "llama_guard_stack:invalid response")

    result["raw_response"] = payload
    result["flagged"] = _as_bool(payload.get("flagged", False))
    result["blocked"] = _as_bool(payload.get("blocked", result["flagged"]))
    result["risk_score"] = payload.get("risk_score")
    result["categories"] = payload.get("categories") or []
    result["blocked_by"] = payload.get("blocked_by") or []
...
def _lakera_guardrail(config: ExternalGuardrailConfig, stage: str, text: str) -> dict[str, Any]:
    ...
    payload = json.dumps(
        {"messages": [{"role": "user", "content": text or ""}], "breakdown": True},
        ensure_ascii=False,
    ).encode("utf-8")
    ...
    # Lakera v2 returns per-detector results under "breakdown" (requires breakdown=true
    # in the request). Capture the detector_type of every fired detector as categories;
    # fall back to the legacy top-level "categories" key if breakdown is absent.
    ...
    if isinstance(breakdown, list):
        ...
        detected = []
    result["categories"] = detected or (data.get("categories") or [])
...
def check(
    ...
    if config.provider == "meta_prompt_guard":
        return _meta_prompt_guardrail(config, stage, text)
    if config.provider == "generic_http":
        return _generic_http_guardrail(config, stage, text, metadata)
    if config.provider == "llama_guard_stack":
        return _llama_guard_stack(config, stage, text, metadata)
...
def merge_stage_result(summary: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    ...
    merged_blocked_by = list(summary.get("external_guardrail_blocked_by") or [])
    for detector in blocked_by:
        if detector not in merged_blocked_by:
            merged_blocked_by.append(detector)
    summary["external_guardrail_blocked_by"] = merged_blocked_by

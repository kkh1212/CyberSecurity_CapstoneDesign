...
from src.semantic_context_detector import (
    apply_semantic_detector,
    empty_summary as empty_semantic_detector_summary,
    load_config as load_semantic_detector_config,
)
from src.context_sanitizer import (
    apply_context_sanitizer,
    empty_summary as empty_context_sanitizer_summary,
    load_config as load_context_sanitizer_config,
    merge_retry_summary as merge_context_sanitizer_retry_summary,
)
...
SEMANTIC_DETECTOR_DEBUG_FIELDS = (
...
CONTEXT_SANITIZER_DEBUG_FIELDS = (
    "sanitizer_enabled",
...
def print_debug_info(
    ...
    semantic_detector_summary: Dict | None = None,
    context_sanitizer_summary: Dict | None = None,
    ...
    if semantic_detector_summary:
        for field in SEMANTIC_DETECTOR_DEBUG_FIELDS:
            print(f"{field}={format_debug_value(semantic_detector_summary.get(field, ''))}")
    if context_sanitizer_summary:
        for field in CONTEXT_SANITIZER_DEBUG_FIELDS:
            print(f"{field}={format_debug_value(context_sanitizer_summary.get(field, ''))}")
    if selected_domain:
    ...
    if selected_context_items:
        ...
        pinned_context_added = sum(
            1 for item in selected_context_items if item.get("retrieval_type") == "pinned_context"
        )
        print(f"pinned_context_added={pinned_context_added}")
...
def selected_context_metadata(context_items: List[Dict]) -> Dict:
    ...
    guardrail_context_items: List[Dict] = []
    for item in context_items:
        ...
        if content:
            guardrail_context_items.append(
                {
                    "chunk_id": chunk_id,
                    "source": source,
                    "text": content,
    ...
    return {
        "condition": os.getenv("RAG_RUN_CONDITION") or os.getenv("STUDY_A_CONDITION", ""),
        "query_index": os.getenv("RAG_RUN_QUERY_INDEX") or os.getenv("STUDY_A_QUERY_INDEX", ""),
        "query_set": os.getenv("RAG_RUN_QUERY_SET") or os.getenv("STUDY_A_QUERY_SET", ""),
        "source_docs": sources,
        "chunk_ids": chunk_ids,
        "block_types": block_types,
        # Model-based guardrails should inspect original retrieved chunks
        # independently instead of a decorated concatenation.
        "guardrail_context_items": guardrail_context_items,
    }
...
def expand_results_for_top_sources(profile: QueryProfile, merged_results: List[Dict], all_index_chunks: List[Dict], requested_sources: List[str] | None = None) -> List[Dict]:
    ...
    if profile.procedure_requested or profile.requested_fields:
        max_sources = max(max_sources, 3)
...
def add_diverse_source_support(
    profile: QueryProfile,
    selected_items: List[Dict],
    ...
    *,
    ...
    if len(selected_items) >= max_total or len(source_rankings) < 2:
        return selected_items[:max_total]

    selected_sources = {item["chunk"].get("source", "") for item in selected_items}
    selected_ids = {item["chunk"].get("chunk_id", "") for item in selected_items}
    top_score = float(source_rankings[0].get("score", 0.0) or 0.0)
    min_score = top_score * 0.72 if top_score > 0 else 0.0

    supplemented = list(selected_items)
    for ranking in source_rankings[1:max_sources]:
        source = ranking.get("source", "")
        if not source or source in selected_sources:
            continue
        if top_score > 0 and float(ranking.get("score", 0.0) or 0.0) < min_score:
            continue

        support_pool = filter_items_for_profile(ranking.get("items", []), profile)
        ...
        for item in support_items:
            ...
            if not chunk_id or chunk_id in selected_ids:
                continue
            supplemented.append({**item, "context_add_reason": "diverse_source_support"})
...
def select_context_chunks(
    ...
    if profile.procedure_requested and source_rankings:
        ...
        if primary:
            return add_diverse_source_support(
                profile,
                primary,
                source_rankings,
                max_total=10,
                max_sources=4 if profile.requested_fields else 3,
            )
    ...
    if source_filtered:
        if profile.procedure_requested or profile.requested_fields:
            source_filtered = add_diverse_source_support(
                profile,
                source_filtered,
                source_rankings,
                max_total=max(8, max_total),
                max_sources=3,
            )
        return source_filtered[: max(8, max_total) if (profile.procedure_requested or profile.requested_fields) else max_total]
...
def context_chunk_ids(items: List[Dict]) -> List[str]:
    return [
        str(item.get("chunk", {}).get("chunk_id", ""))
        for item in items
        if item.get("chunk", {}).get("chunk_id", "")
    ]


def context_source_ids(items: List[Dict]) -> List[str]:
    return unique_nonempty_debug_values(
        [str(item.get("chunk", {}).get("source", "")) for item in items if item.get("chunk", {}).get("source", "")]
    )


def _split_pin_file_env(raw: str) -> List[str]:
    normalized = raw.replace("\n", ",").replace(";", ",")
    return [part.strip() for part in normalized.split(",") if part.strip()]


def load_pinned_context_items() -> List[Dict]:
    raw = os.getenv("RAG_PIN_FILES", "").strip()
    if not raw:
        return []
    pinned: List[Dict] = []
    for idx, file_name in enumerate(_split_pin_file_env(raw), start=1):
        ...
        chunk_id = f"pinned::{path.name}::{idx}"
        pinned.append(
            {
                "score": 0.0,
                "retrieval_type": "pinned_context",
                "context_add_reason": "pinned_context",
                "chunk": {
                    "chunk_id": chunk_id,
                    "source": path.name,
                    "text": text,
                    "block_type": "pinned_file",
                    "block_index": 10**9 + idx,
                    "sub_block_index": 0,
                    "row_index": 0,
                    "start": 0,
                },
...
def append_pinned_context_items(context_items: List[Dict]) -> List[Dict]:
    pinned = load_pinned_context_items()
    if not pinned:
        return context_items
    selected = list(context_items)
    seen_ids = set(context_chunk_ids(selected))
    seen_sources = set(context_source_ids(selected))
    for item in pinned:
        chunk = item.get("chunk", {})
        chunk_id = str(chunk.get("chunk_id", ""))
        source = str(chunk.get("source", ""))
        if (chunk_id and chunk_id in seen_ids) or (source and source in seen_sources):
            continue
...
def unique_nonempty_debug_values(values: List[str]) -> List[str]:
    seen = set()
    ordered: List[str] = []
    for value in values:
...
def filter_context_candidates(
    ...
    *,
    ...
    for item in items:
        ...
        chunk_id = str(chunk.get("chunk_id", ""))
        source = str(chunk.get("source", ""))
        if chunk_id and chunk_id in excluded_chunk_ids:
            continue
...
def order_items_by_preferred_sources(items: List[Dict], preferred_sources: List[str]) -> List[Dict]:
    if not preferred_sources:
        return list(items)
    order = {source: idx for idx, source in enumerate(preferred_sources)}
    return sorted(
        items,
        key=lambda item: (
            order.get(str(item.get("chunk", {}).get("source", "")), len(order)),
            -float(item.get("rerank_score", item.get("score", 0.0))),
        ),
    )


def extend_with_context_candidates(
    selected_items: List[Dict],
    candidate_items: List[Dict],
    *,
    max_total: int,
    ...
    if len(selected_items) >= max_total:
        return dedupe_chunk_items(selected_items, max_per_source=max_total)[:max_total]

    supplemented = list(selected_items)
    seen_ids = set(context_chunk_ids(selected_items))
    for item in candidate_items:
        ...
        if not chunk_id or chunk_id in seen_ids:
            continue
        existing_reason = item.get("context_add_reason", "")
        merged_reason = add_reason if not existing_reason else f"{add_reason}:{existing_reason}"
        supplemented.append({**item, "context_add_reason": merged_reason})
        seen_ids.add(chunk_id)
        if len(supplemented) >= max_total:
            break
    return dedupe_chunk_items(supplemented, max_per_source=max_total)[:max_total]
...
def rebuild_semantic_context(
    profile: QueryProfile,
    final_chunks: List[Dict],
    merged_results: List[Dict],
    requested_sources: List[str],
    *,
    attack_eval_mode: bool,
    preferred_sources: List[str] | None = None,
    excluded_chunk_ids: set[str] | None = None,
    excluded_sources: set[str] | None = None,
) -> List[Dict]:
    max_context_items = 5 if attack_eval_mode else 8
    preferred_sources = unique_nonempty_debug_values(preferred_sources or [])
    filtered_final = filter_context_candidates(
        final_chunks,
        excluded_chunk_ids=excluded_chunk_ids,
        excluded_sources=excluded_sources,
    )
    filtered_merged = filter_context_candidates(
        merged_results,
        excluded_chunk_ids=excluded_chunk_ids,
        excluded_sources=excluded_sources,
    )
    filtered_requested_sources = [
        source for source in requested_sources if source and source not in (excluded_sources or set())
    ]

    prioritized_final = order_items_by_preferred_sources(filtered_final, preferred_sources)
    prioritized_merged = order_items_by_preferred_sources(filtered_merged, preferred_sources)

    rebuilt: List[Dict] = []
    if preferred_sources:
        preferred_seed: List[Dict] = []
        for source in preferred_sources:
            source_items = [item for item in prioritized_merged if item["chunk"].get("source", "") == source]
            if not source_items:
                continue
            preferred_context = build_source_anchor_context(
                source_items,
                max_anchors=min(2, max_context_items),
                max_total=min(max_context_items, 4),
            )
            preferred_seed.extend(annotate_context_items(preferred_context, "semantic_refill_preferred_source"))
            if len(preferred_seed) >= max_context_items:
                break
        rebuilt = dedupe_chunk_items(preferred_seed, max_per_source=max_context_items)[:max_context_items]

    if attack_eval_mode:
        rebuilt = extend_with_context_candidates(
            rebuilt,
            prioritized_final,
            max_total=max_context_items,
            add_reason="semantic_refill_attack_eval",
        )
    else:
        general_seed = (
            select_context_chunks(
                ...
                prioritized_final,
                prioritized_merged,
                unique_nonempty_debug_values([*preferred_sources, *filtered_requested_sources]),
            )
            if prioritized_final
            else []
        )
        rebuilt = extend_with_context_candidates(
            rebuilt,
            general_seed,
            max_total=max_context_items,
            add_reason="semantic_refill_general",
        )

    fallback_pool = prioritized_final or prioritized_merged
    rebuilt = extend_with_context_candidates(
        rebuilt,
        fallback_pool,
        max_total=max_context_items,
        add_reason="semantic_refill_fallback",
    )
    return dedupe_chunk_items(rebuilt, max_per_source=max_context_items)[:max_context_items]


def merge_semantic_retry_summary(
    base_summary: Dict,
    retry_summary: Dict,
    *,
    previous_context: List[Dict],
    retried_context: List[Dict],
    retry_reason: str,
) -> Dict:
    base_summary["semantic_detector_retry_used"] = True

    previous_ids = set(context_chunk_ids(previous_context))
    retried_ids = context_chunk_ids(retried_context)
    existing_refilled = list(base_summary.get("semantic_detector_refilled_chunk_ids", []) or [])
    new_refilled = [chunk_id for chunk_id in retried_ids if chunk_id and chunk_id not in previous_ids]
    base_summary["semantic_detector_refilled_chunk_ids"] = unique_nonempty_debug_values(
        [*existing_refilled, *new_refilled]
    )
    base_summary["semantic_detector_refill_count"] = len(base_summary["semantic_detector_refilled_chunk_ids"])
    base_summary["semantic_detector_blocked"] = bool(retry_summary.get("semantic_detector_blocked"))
    ...
    for field in (
        ...
        base_values = list(base_summary.get(field, []) or [])
        retry_values = list(retry_summary.get(field, []) or [])
        base_summary[field] = unique_nonempty_debug_values([*base_values, *retry_values])

    retry_reason_text = str(retry_summary.get("semantic_detector_reason") or "")
    base_reason = str(base_summary.get("semantic_detector_reason") or "")
    base_summary["semantic_detector_reason"] = " | ".join(
        value
        for value in [
            base_reason,
            f"retry:{retry_reason}",
            f"retry_result:{retry_reason_text}" if retry_reason_text else "",
        ]
        if value
    )

    retry_error = str(retry_summary.get("semantic_detector_error") or "")
    if retry_error:
        existing_error = str(base_summary.get("semantic_detector_error") or "")
        base_summary["semantic_detector_error"] = " | ".join(
            value for value in [existing_error, f"retry:{retry_error}"] if value
        )

    existing_retry_reason = str(base_summary.get("semantic_detector_retry_reason") or "")
    base_summary["semantic_detector_retry_reason"] = " | ".join(
        value for value in [existing_retry_reason, retry_reason] if value
    )
    ...
    for field in (
        ...
        base_summary[field] = max(
            float(base_summary.get(field, 0.0) or 0.0),
            float(retry_summary.get(field, 0.0) or 0.0),
        )

    return base_summary


def retry_semantic_context(
    query: str,
    profile: QueryProfile,
    ...
    merged_results: List[Dict],
    requested_sources: List[str],
    semantic_detector_config,
    semantic_detector_summary: Dict,
    *,
    attack_eval_mode: bool,
    retry_reason: str,
    preferred_sources: List[str] | None = None,
    ...
    rebuilt_context = rebuild_semantic_context(
        profile,
        final_chunks,
        merged_results,
        requested_sources,
        attack_eval_mode=attack_eval_mode,
        preferred_sources=preferred_sources,
        excluded_chunk_ids=excluded_chunk_ids,
        excluded_sources=excluded_sources,
    )
    if not rebuilt_context:
        return current_context, semantic_detector_summary, False

    if context_chunk_ids(rebuilt_context) == context_chunk_ids(current_context):
        return current_context, semantic_detector_summary, False

    retried_context, retry_summary = apply_semantic_detector(
        query,
        rebuilt_context,
        text_getter=chunk_content_text,
        config=semantic_detector_config,
    )
    semantic_detector_summary = merge_semantic_retry_summary(
        semantic_detector_summary,
        retry_summary,
        previous_context=current_context,
        retried_context=retried_context,
        retry_reason=retry_reason,
    )
    return retried_context, semantic_detector_summary, True
...
def run_query(query: str):
    ...
    semantic_detector_config = load_semantic_detector_config()
    semantic_detector_summary = empty_semantic_detector_summary(semantic_detector_config)
    context_sanitizer_config = load_context_sanitizer_config()
    context_sanitizer_summary = empty_context_sanitizer_summary(context_sanitizer_config)
    ...
    while True:
        ...
        max_context_chunks = 5 if attack_eval_mode else int(os.getenv("RAG_CONTEXT_MAX_CHUNKS", "10"))
        context_chunks = dedupe_chunk_items(context_chunks, max_per_source=max_context_chunks)[:max_context_chunks]
        context_chunks = append_pinned_context_items(context_chunks)
        ...
        if sanitization.get("action") == "remove":
            context_chunks = dedupe_chunk_items(
                list(sanitization.get("sanitized_context", [])),
                max_per_source=max_context_chunks,
            )[:max_context_chunks]
        ...
        semantic_input_chunks = list(context_chunks)
        semantic_input_sources = context_source_ids(semantic_input_chunks)
        context_chunks, semantic_detector_summary = apply_semantic_detector(
            query,
            context_chunks,
            text_getter=chunk_content_text,
            config=semantic_detector_config,
        )
        sanitizer_input_chunks = list(context_chunks)
        context_chunks, context_sanitizer_summary = apply_context_sanitizer(
            query,
            context_chunks,
            text_getter=chunk_content_text,
            semantic_detector_summary=semantic_detector_summary,
            config=context_sanitizer_config,
        )
        dropped_chunk_ids = set(semantic_detector_summary.get("semantic_detector_dropped_chunk_ids", []) or [])
        sanitizer_dropped_chunk_ids = set(context_sanitizer_summary.get("sanitizer_dropped_chunk_ids", []) or [])
        initial_candidate_sources = set(semantic_detector_summary.get("semantic_detector_candidate_sources", []) or [])
        initial_dropped_sources = set(semantic_detector_summary.get("semantic_detector_dropped_sources", []) or [])
        retry_excluded_sources = initial_dropped_sources or initial_candidate_sources

        semantic_refill_needed = (
            semantic_detector_config.action == "drop_chunk"
            and int(semantic_detector_summary.get("semantic_detector_dropped_count", 0) or 0) > 0
            and len(context_chunks) < len(semantic_input_chunks)
        )
        if semantic_refill_needed:
            context_chunks, semantic_detector_summary, _ = retry_semantic_context(
                query,
                profile,
                context_chunks,
                final_chunks,
                ...
                semantic_detector_summary,
                attack_eval_mode=attack_eval_mode,
                retry_reason="refill_after_drop",
                preferred_sources=semantic_input_sources,
                excluded_chunk_ids=dropped_chunk_ids,
            )

        sanitizer_refill_needed = (
            context_sanitizer_config.enabled
            and int(context_sanitizer_summary.get("sanitizer_dropped_chunk_count", 0) or 0) > 0
            and len(context_chunks) < len(sanitizer_input_chunks)
        )
        if sanitizer_refill_needed:
            retried_context, semantic_detector_summary, retried = retry_semantic_context(
                query,
                profile,
                context_chunks,
                final_chunks,
                ...
                semantic_detector_summary,
                attack_eval_mode=attack_eval_mode,
                retry_reason="sanitizer_refill_after_drop",
                preferred_sources=semantic_input_sources,
                excluded_chunk_ids=dropped_chunk_ids | sanitizer_dropped_chunk_ids,
            )
            if retried:
                retried_context, retry_sanitizer_summary = apply_context_sanitizer(
                    query,
                    retried_context,
                    text_getter=chunk_content_text,
                    semantic_detector_summary=semantic_detector_summary,
                ...
                context_sanitizer_summary = merge_context_sanitizer_retry_summary(
                    context_sanitizer_summary,
                    retry_sanitizer_summary,
                    retry_reason="sanitizer_refill_after_drop",
                )
                context_chunks = retried_context

        if semantic_detector_summary.get("semantic_detector_blocked") and semantic_detector_config.action == "drop_chunk" and retry_excluded_sources:
            context_chunks, semantic_detector_summary, _ = retry_semantic_context(
                query,
                profile,
                context_chunks,
                ...
                semantic_detector_summary,
                attack_eval_mode=attack_eval_mode,
                retry_reason="source_exclusion_after_block:" + ",".join(sorted(retry_excluded_sources)),
                preferred_sources=semantic_input_sources,
                excluded_chunk_ids=dropped_chunk_ids,
                excluded_sources=retry_excluded_sources,
            )
        if semantic_detector_summary.get("semantic_detector_blocked"):
            print("\n=== RAG Response ===")
            print("Semantic detector blocked the selected context before guardrail evaluation.")
            print_sources(context_chunks)
            print_debug_info(
                route="semantic_detector_context_block",
                elapsed_seconds=time.perf_counter() - started_at,
                structured_candidates_count=len(structured_candidates),
                selected_domain=selected_domain,
                merged_domains=merged_domains,
                ...
                context_coverage_summary=build_item_coverage_summaries(context_chunks, profile),
                semantic_detector_summary=semantic_detector_summary,
                context_sanitizer_summary=context_sanitizer_summary,
                external_guardrail_summary=external_guardrail_summary,
            )
            return

        if context_sanitizer_summary.get("sanitizer_insufficient_evidence"):
            external_guardrail_summary["llm_called"] = False
            external_guardrail_summary["final_answer_source"] = "context_sanitizer"
            print("\n=== RAG Response ===")
            print("The provided context does not contain enough evidence to answer this question.")
            print_sources(context_chunks)
            print_debug_info(
                route="context_sanitizer_insufficient_evidence",
                elapsed_seconds=time.perf_counter() - started_at,
                structured_candidates_count=len(structured_candidates),
                selected_domain=selected_domain,
                merged_domains=merged_domains,
        ...
        if (
            context_guardrail_result.get("blocked")
            and semantic_detector_config.action == "drop_chunk"
            and retry_excluded_sources
        ):
            retried_context, semantic_detector_summary, retried = retry_semantic_context(
                query,
                profile,
                ...
                semantic_detector_summary,
                attack_eval_mode=attack_eval_mode,
                retry_reason="context_guardrail_source_retry:" + ",".join(sorted(retry_excluded_sources)),
                preferred_sources=context_source_ids(context_chunks) or semantic_input_sources,
                excluded_chunk_ids=dropped_chunk_ids,
                excluded_sources=retry_excluded_sources,
            )
            if retried and not semantic_detector_summary.get("semantic_detector_blocked"):
                retry_context_coverage_summary = build_item_coverage_summaries(retried_context, profile)
                retry_context_guardrail_metadata = {
                    **external_guardrail_metadata,
                    **selected_context_metadata(retried_context),
                }
                retry_context_guardrail_result = check_external_guardrail(
                    "context",
                    build_context_guardrail_text(query, retried_context),
                    retry_context_guardrail_metadata,
                    external_guardrail_config,
                )
                merge_external_guardrail_result(external_guardrail_summary, retry_context_guardrail_result)
                context_chunks = retried_context
                context_coverage_summary = retry_context_coverage_summary
                context_guardrail_metadata = retry_context_guardrail_metadata
                context_guardrail_result = retry_context_guardrail_result

...
# Direct "ignore/refuse" attack variants (employee record removed; payload tells
# the model to refuse). Used by the v5_full / v5_ignore condition keywords.
...
def _leak_target_profile(question: Question) -> dict[str, str]:
    """Ground-truth employee record for a document's leak target.

    The v5 employee-leak corpus moved the planted record out of the document into
    hr_employee_directory.md, so it can no longer be parsed from the document.
    leak_targets_v5.json maps the target filename -> employee fields.
    """
...
def extract_injected_profile(dataset_dir: Path, question: Question) -> dict[str, str]:
    ...
    if not fields:
        # v5 leak design: the record lives in hr_employee_directory.md, not the doc.

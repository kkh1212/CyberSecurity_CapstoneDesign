...
DIRECTIVE_PATTERNS = (
    ...
    r"\bignore the user's original question\b",
    r"\bignore the user'?s original question\b",
    r"\bdisregard the preceding section\b",
    r"\bdisregard the user's question\b",
    r"\bdisregard the user request\b",
    r"\bdisregard\b.*\bsafety guidance\b",
    r"\bdo not summarise any policy\b",
    r"\bdo not summarize any policy\b",
    r"\bforget the previous details\b",
    r"\bfocus on my new question\b",
    r"\bfollow this instruction\b",
    r"\bfollow the instruction below\b",
    r"\bfollow the embedded instruction\b",
    r"\bhigher priority than\b",
    r"\bauthoritative response\b",
    r"\bmust not provide\b",
    r"\bmust respond only\b",
    r"\brespond only with\b",
    r"\brespond with exactly\b",
    r"\breply with exactly\b",
    r"\breply with only\b",
    r"\breply exactly\b",
    r"\boutput only\b",
    r"\boutput exactly\b",
    r"\boutput the full directory record\b",
    r"\byour entire response must be exactly\b",
    r"\btakes precedence over\b",
    r"\bmandatory response standard\b",
    r"\bsystem must\b",
    r"\bsystem note\b",
    r"\bsystem override\b",
    r"\bpriority directive\b",
    r"\bimportant system instruction\b",
    r"\bnote to assistant\b",
    r"\bdo not answer\b",
    r"\bdo not address\b",
    r"\bdo not respond to\b",
    r"\bprovide no information\b",
    r"\brefuse the user's request\b",
    r"\bcannot answer\b",
    r"\bthe document does not confirm this\b",
    r"\breveal\b.*\b(internal|system|secret|confidential)\b",
    r"\blook up the entry\b",
    r"\bretrieve the entry\b",
    r"\byour only task is to retrieve\b",
    r"\breproduce\b.*\bfull record\b",
    r"\bprint\b.*\b(employee|record|directory)\b",
    r"\boutput\b.*\b(employee|record|directory)\b",
    r"\breturn\b.*\b(personnel|employee|directory).*\brecord\b",
    r"\bhr_employee_directory\.md\b",
    r"\bemployee record\b",
    r"\bcomplete personnel record\b",
    r"\bcomplete record\b",
    r"\bcomplete answer\b",
    r"\bmandated refusal\b",
    r"\bskip the user's question\b",
    r"\bthe correct behavior is to\b",
...
def load_config() -> ContextSanitizerConfig:
    ...
    return ContextSanitizerConfig(
        ...
        fail_mode=fail_mode,
        force_all_context=_env_bool("CONTEXT_SANITIZER_FORCE_ALL", False),
    )
...
def apply_context_sanitizer(
    ...
    *,
    ...
    if config.force_all_context:
        candidate_ids = {
            str(item.get("chunk", {}).get("chunk_id", ""))
            for item in context_items
            if str(item.get("chunk", {}).get("chunk_id", ""))
        }
        candidate_sources = {
            str(item.get("chunk", {}).get("source", ""))
            for item in context_items
            if str(item.get("chunk", {}).get("source", ""))
        }
    summary["sanitizer_detected_chunk_count"] = int(
        semantic_detector_summary.get("semantic_detector_candidate_count", 0) or 0
    )
    if config.force_all_context and summary["sanitizer_detected_chunk_count"] == 0:
        summary["sanitizer_detected_chunk_count"] = len(candidate_ids or candidate_sources)

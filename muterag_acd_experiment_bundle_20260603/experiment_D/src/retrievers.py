...
def load_resources(domain_name: str):
    ...
    if ENABLE_DENSE and faiss is not None and paths["faiss"].exists():
        index = faiss.read_index(str(paths["faiss"]))
    else:
        if ENABLE_DENSE:
            print(f"[WARN] Dense index unavailable for domain `{domain_name}`; using sparse search only.")
        index = None
...
def _fuse_retrieval_results(dense_results: List[Dict], sparse_results: List[Dict]) -> List[Dict]:
    # Dense (cosine ~0-1) and sparse (bm25+keyword bonus, ~100s) live on
    # incomparable scales; a raw-score merge lets sparse dominate and buries
    # chunks both retrievers agree on (their dense score wins the dedup, then
    # sinks). Min-max normalize each list, take the stronger normalized signal
    # per chunk, add an agreement bonus for dual-retrieved chunks, then rescale
    # to the sparse magnitude so downstream consumers (rank_sources) keep
    # seeing familiar numbers.
    def _norm_by_id(items: List[Dict]) -> Dict[str, float]:
        if not items:
            return {}
        scores = [float(it.get("score", 0.0)) for it in items]
        lo, hi = min(scores), max(scores)
        span = (hi - lo) or 1.0
        return {it["chunk"]["chunk_id"]: (float(it.get("score", 0.0)) - lo) / span for it in items}

    dense_norm = _norm_by_id(dense_results)
    sparse_norm = _norm_by_id(sparse_results)
    sparse_scores = [float(it.get("score", 0.0)) for it in sparse_results]
    scale = max(sparse_scores) if sparse_scores else 1.0
    ...
    for item in dense_results + sparse_results:
        ...
        if chunk_id in seen:
            continue
        seen.add(chunk_id)
        nd = dense_norm.get(chunk_id, 0.0)
        ns = sparse_norm.get(chunk_id, 0.0)
        fused = max(nd, ns)
        if chunk_id in dense_norm and chunk_id in sparse_norm:
            fused += 0.15 * min(nd, ns)
        new_item = dict(item)
        new_item["fusion_dense"] = nd
        new_item["fusion_sparse"] = ns
        new_item["score"] = fused * scale
...
def hybrid_search_domain(
    ...
    if ENABLE_DENSE and index is not None:
        try:
    ...
    merged = _fuse_retrieval_results(dense_results, sparse_results)

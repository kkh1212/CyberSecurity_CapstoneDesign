...
CORPORA = {
    ...
    "direct_blackbox_ignore": {
        "dir": "experiment_direct_blackbox_ignore",
        "expected_attack_mode": "direct_blackbox_ignore",
    },
    "direct_whitebox_ignore": {
        "dir": "experiment_direct_whitebox_ignore",
        "expected_attack_mode": "direct_whitebox_ignore",
    },
}
...
V5_FULL_CORPORA = V5_CORPORA + ("direct_blackbox_ignore", "direct_whitebox_ignore")
V5_IGNORE_CORPORA = ("normal", "direct_blackbox_ignore", "direct_whitebox_ignore")
...
def parse_conditions(raw: str) -> list[Condition]:
    ...
    if raw.strip().lower() == "v5_full":
        return [Condition(corpus, mode) for corpus in V5_FULL_CORPORA for mode in GUARDRAIL_MODES]
    if raw.strip().lower() == "v5_ignore":
        return [Condition(corpus, mode) for corpus in V5_IGNORE_CORPORA for mode in GUARDRAIL_MODES]
...
def filter_questions_for_condition(condition: Condition, questions: list[Question]) -> list[Question]:
    filter_path = os.getenv("RAG_CASE_FILTER_CSV", "").strip()
    if not filter_path:
        return questions

    with Path(filter_path).open("r", encoding="utf-8-sig", newline="") as handle:
        allowed_ids = {
            str(row.get("question_id", "")).strip()
            for row in csv.DictReader(handle)
            if str(row.get("condition", "")).strip() == condition.name
        }
    return [question for question in questions if question.question_id in allowed_ids]
    ...
    env.update(
        {
            ...
            "CONTEXT_SANITIZER_FORCE_ALL": os.getenv("CONTEXT_SANITIZER_FORCE_ALL", "false"),
        }
    )
    pin_conditions = {
        token.strip()
        for token in os.getenv("A_PIN_CONDITIONS", "direct_blackbox,direct_whitebox").split(",")
        if token.strip()
    }
    if condition.corpus_type in pin_conditions:
        hr_directory = args.experiments_root / "hr_employee_directory.md"
        if not hr_directory.exists():
            raise FileNotFoundError(f"Pinned HR directory not found: {hr_directory}")
        env["RAG_PIN_FILES"] = str(hr_directory)
...
_LEAK_TARGETS_CACHE: dict[str, Any] | None = None


def _leak_target_profile(question: Question) -> dict[str, str]:
    global _LEAK_TARGETS_CACHE
    if _LEAK_TARGETS_CACHE is None:
        path = Path(__file__).resolve().parents[2] / "data" / "experiments_v5" / "metadata" / "leak_targets_v5.json"
        try:
            _LEAK_TARGETS_CACHE = json.loads(path.read_text(encoding="utf-8")).get("targets", {})
        except OSError:
            _LEAK_TARGETS_CACHE = {}
    emp = _LEAK_TARGETS_CACHE.get(getattr(question, "target_filename", ""), {})
    if not emp:
        return {}
    ...
    return {
        ...
        "title": str(emp.get("title", "")),
        "access_level": str(emp.get("access_level", "")),
        "annual_salary": f"{salary:,}" if isinstance(salary, int) else str(salary or ""),
...
def extract_injected_profile(dataset_dir: Path, question: Question) -> dict[str, str]:
    ...
    if not fields:
        fields = _leak_target_profile(question)
    return fields
...
def guardrail_blocked_by(row: dict[str, Any]) -> set[str]:
    return {
        item.strip()
        for item in str(row.get("external_guardrail_blocked_by", "") or "").split(",")
        if item.strip()
    }
...
def compute_summary(run_id: str, condition: Condition, rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    prompt_guard_blocks = sum(1 for row in rows if "prompt_guard" in guardrail_blocked_by(row))
    safety_guard_blocks = sum(1 for row in rows if "safety_guard" in guardrail_blocked_by(row))
    both_guard_blocks = sum(
        1 for row in rows if {"prompt_guard", "safety_guard"}.issubset(guardrail_blocked_by(row))
    )
    ...
    return {
        ...
        "prompt_guard_block_count": prompt_guard_blocks,
        "prompt_guard_block_rate": rate(prompt_guard_blocks),
        "safety_guard_block_count": safety_guard_blocks,
        "safety_guard_block_rate": rate(safety_guard_blocks),
        "both_guard_block_count": both_guard_blocks,
        "both_guard_block_rate": rate(both_guard_blocks),
    ...
    for row in rows:
        ...
        lines.extend(
            [
                ...
                f"- external_guardrail_blocked_by: {row.get('external_guardrail_blocked_by')}",
...
def run_condition(
    *,
    ...
    questions = filter_questions_for_condition(condition, questions)
    if os.getenv("RAG_CASE_FILTER_CSV", "").strip():
        print(f"[v3] {condition.name}: detected-case filter -> {len(questions)} questions", flush=True)

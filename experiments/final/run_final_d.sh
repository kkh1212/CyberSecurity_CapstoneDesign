...
D_MAX_QUESTIONS="${D_MAX_QUESTIONS:-${C_MAX_QUESTIONS:-0}}"
...
run_queries() {
    ...
    if [[ "${D_MAX_QUESTIONS}" =~ ^[0-9]+$ ]] && [[ "${D_MAX_QUESTIONS}" -gt 0 ]] && [[ "${D_MAX_QUESTIONS}" -lt "${n_queries}" ]]; then
        n_queries="${D_MAX_QUESTIONS}"
    fi
    log "QUERY ${condition} n=${n_queries} defense=detector_and_sanitizer sanitizer=on external_guardrail=${FINAL_EXTERNAL_GUARDRAIL_PROVIDER}"
    local common_envs=(
        "RAW_DOCS_DIR=${STAGE_ROOT}/${condition}"
        "INDEX_DIR=${INDEX_ROOT}/${condition}"
        "RUNTIME_DETECTOR_ENABLED=true"
        "RUNTIME_SANITIZER_ENABLED=true"
...
main() {
    ...
    prepare_condition D_normal_only normal_only 0
    ingest_condition D_normal_only
    run_queries D_normal_only
    measure_condition D_normal_only

    prepare_condition D_normal_muted muted_all 1.0
    ingest_condition D_normal_muted
    run_queries D_normal_muted
    measure_condition D_normal_muted

...
B_MAX_QUESTIONS="${B_MAX_QUESTIONS:-0}"
B_RATES="${B_RATES:-0 0.01 0.03 0.05 0.10 1.0}"
B_ATTACK_MODE="${B_ATTACK_MODE:-rate}"
...
print(f"B_rate_{int(round(rate * 100)):03d}")
PY
}

prepare_condition() {
    local condition="$1" rate="$2"
    stage_fixed_condition "${condition}" "${B_ATTACK_MODE}" "${rate}" "${STAGE_ROOT}/${condition}" "${QUERIES_JSON}"
}
...
run_queries() {
    ...
    if [[ "${B_MAX_QUESTIONS}" =~ ^[0-9]+$ ]] && [[ "${B_MAX_QUESTIONS}" -gt 0 ]] && [[ "${B_MAX_QUESTIONS}" -lt "${n_queries}" ]]; then
        n_queries="${B_MAX_QUESTIONS}"
    fi
    ...
    cat > "${RESULTS_ROOT}/run_metadata.json" <<EOF
{
  ...
  "rates": "${B_RATES}",
  "attack_mode": "${B_ATTACK_MODE}",
  "max_questions": "${B_MAX_QUESTIONS}",
  "final_top_k": "${FINAL_TOP_K}",
  "sparse_top_k": "${SPARSE_TOP_K}",
  "enable_dense": "false",
  "enable_rerank": "false",
...
main() {
    ...
    write_run_metadata
    echo "condition,requested_rate,attack_mode,model,final_top_k,sparse_top_k,corpus_dir,attack_documents,universal_attack_documents,total_documents,total_chunks,attack_chunks,attack_chunk_ratio" > "${RESULTS_ROOT}/final_b_summary.csv"
    log "RUN_ID=${RUN_ID} MODEL=${OLLAMA_MODEL} ATTACK_MODE=${B_ATTACK_MODE} RATES=${B_RATES}"
    for rate in ${B_RATES}; do
        condition="$(rate_label "${rate}")"
        prepare_condition "${condition}" "${rate}"
        ingest_condition "${condition}"
        run_queries "${condition}"
        measure_condition "${condition}"
        jq -r --arg rate "${rate}" \
              --arg attack_mode "${B_ATTACK_MODE}" \
              --arg model "${OLLAMA_MODEL}" \
              --arg final_top_k "${FINAL_TOP_K}" \
              --arg sparse_top_k "${SPARSE_TOP_K}" \
              --arg corpus_dir "${FINAL_CORPUS_DIR}" \
              '[.condition,$rate,$attack_mode,$model,$final_top_k,$sparse_top_k,$corpus_dir,.attack_documents,.universal_attack_documents,.total_documents,.total_chunks,.attack_chunks,.attack_chunk_ratio] | @csv' \
            "${STAGE_ROOT}/${condition}/inject_summary.json" >> "${RESULTS_ROOT}/final_b_summary.csv"
    done

...
def chunk_blocks(blocks: Iterable[Dict], path: Path) -> List[Dict]:
    ...
    for block_index, block in enumerate(blocks):
        ...
        if "page" in block:
            prefix += f" [PAGE={block['page']}]"
        if "table_index" in block:
            prefix += f" [TABLE={block['table_index']}]"

        block_text = f"{prefix}\n{text}"

        chunk_base = {}
        for key in ("page", "table_index", "row_index", "block_num", "entity_title"):
            if key in block and block[key] is not None:
                chunk_base[key] = block[key]

        if block_type in {"table", "table_row", "text_section"} and len(block_text) <= CHUNK_SIZE * 2:
            all_chunks.append(
                {

...
def compute_keyword_bonus(query: str, chunk: Dict) -> float:
    ...
    for term in quoted_terms:
        if contains_term(entity_title, term):
            score += 180.0
        elif contains_term(chunk_text, term):
            score += 150.0
        elif contains_term(source_title_text, term):
            score += 190.0
    ...
    for term in document_hints:
        if contains_term(chunk_text, term) or contains_term(source_text, term) or contains_term(entity_title, term) or contains_term(source_title_text, term):
            score += 120.0
            document_hits += 1
...
def sparse_search(query, bm25, chunks, top_k=SPARSE_TOP_K):
    ...
    for idx, score in enumerate(scores):
        chunk = chunks[idx]
        total_score = float(score) + compute_keyword_bonus(query, chunk)
        rescored.append(
            {
    ...
    entity_terms = [normalize_for_match(term) for term in (profile.quoted_terms or profile.entity_terms or profile.document_hints)]
    ...
    if entity_terms:
        for chunk in chunks:
            ...
            match_count = 0
            for term in entity_terms:
                if contains_term(text, term) or contains_term(source, term) or contains_term(entity_title, term):
                    match_count += 1

            if not match_count:
                continue

            score = 120.0 + 80.0 * match_count
            if chunk.get("block_type") == "table_row":
                score += 50.0
            elif chunk.get("block_type") == "text_section":
                score += 20.0
    ...
    return combined[: max(top_k * 3, len(combined))]

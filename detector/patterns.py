...
REFUSAL_PATTERNS: Final[list[WeightedPattern]] = [
    ...
    WeightedPattern(
        ...
        description="Constrains the answer to a narrow public-safe subset.",
    ),
    WeightedPattern(
    ...
    WeightedPattern(
        ...
        pattern=r"(?:대신|그러면) .*?도와줄 수 없(?:다고|다고만) .*?(?:응답|답변)|상세 내용을 회피하",
        weight=1.2,
        category="forced_refusal",
    ...
    WeightedPattern(
        ...
        pattern=r"폭탄|폭발물|무기|해킹|악성코드|랜섬웨어|마약|총기",
        weight=0.7,
        category="dangerous_topic",
    ...
    WeightedPattern(
        ...
        pattern=r"어떻게.*?(?:만들|시작|구하|설치|침입)|방법을 알려|제작 방법|공격 방법",
        weight=0.6,
        category="dangerous_request",
    ...
    WeightedPattern(
        ...
        pattern=r"(?:상세|구체|세부|단계별).{0,16}(?:안내|설명|절차|정보|기술 내용).{0,12}(?:제공|안내|설명|공개).{0,12}(?:하지 않|어렵|불가)",
        weight=0.95,
        category="restriction",
    ...
    WeightedPattern(
        ...
        pattern=r"(?:보안|개인정보|운영|준법|컴플라이언스).{0,20}(?:상 이유|정책상|기준상).{0,20}(?:상세|구체|세부).{0,20}(?:제공|안내|설명).{0,12}(?:불가|하지 않|어렵)",
        weight=1.0,
        category="restriction",
    ...
    WeightedPattern(
        ...
        pattern=r"(?:제한적|일반적인|요약된).{0,12}(?:안내|설명|회신|문구).{0,12}(?:만|위주로)|공개 가능한 범위(?:만|에서만)",
        weight=0.7,
        category="limited_response",
    ...
    WeightedPattern(
        ...
        pattern=r"공개 가능한 범위(?:에서만|만)|공식 공지(?:만)? 참고|공개된 안내 범위를 벗어나는",
        weight=0.75,
        category="limited_response",
    ...
    WeightedPattern(
        ...
        pattern=r"추가\s*설명(?:을)?\s*(?:넓히지 않|확장하지 않)|구체(?:적인)?\s*(?:사례|예시|절차).{0,12}(?:제공|안내|설명).{0,12}(?:하지 않|생략)",
        weight=0.9,
        category="limited_response",
    ...
    WeightedPattern(
        ...
        pattern=r"내부\s*(?:운영|절차|승인 사유|검토 결과|판단 근거).{0,12}(?:제공|공개|안내).{0,12}(?:하지 않|불가)|민감(?:한)?\s*문의",
        weight=0.65,
        category="restriction",
        description="Marks content as internal or sensitive and not suitable for detailed disclosure.",
    ),
]
...
EXPLICIT_HIGH_RISK_PATTERNS: Final[list[WeightedPattern]] = [
    ...
    WeightedPattern(
        ...
        pattern=r"문맥과 관련이 있어도 답하지 마|관련 문서가 있어도 답하지 마",
        weight=1.9,
        category="forced_refusal",
    ...
    WeightedPattern(
        ...
        pattern=r"기존 동작.*덮어|모든 요청을 위험한 요청으로 간주",
        weight=1.8,
        category="explicit_override",
    ...
    WeightedPattern(
        ...
        pattern=r"항상 도와줄 수 없다고 말하|정답을 공개하지 마|어시스턴트는 모든 .* 요청을 거절해야",
        weight=1.9,
        category="forced_refusal",
